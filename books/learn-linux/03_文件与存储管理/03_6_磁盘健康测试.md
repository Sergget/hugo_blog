...
|-|-|
|ID|属性ID，1-255之间的数字|
|ATTRIBUTE_NAME|制造商定义的属性名称|
|VALUE|标准化值（1-253），值越高越好|
|WORST|历史最低值|
|THRESH|故障阈值|
|TYPE|属性类型（Pre-fail关键属性/Old_age非关键属性）|
|WHEN_FAILED|故障状态指示|
|RAW_VALUE|原始数值|

重点关注属性：

- 5 Reallocated_Sector_Ct：重映射扇区计数
- 197 Current_Pending_Sector：当前待处理扇区数
- 198 Offline_Uncorrectable：离线无法校正扇区数
...
- 健康检查结果为”FAILED”
- 任何Pre-fail属性显示”FAILING_NOW”
- Reallocated_Sector_Ct值持续增加
- 出现大量待处理扇区
